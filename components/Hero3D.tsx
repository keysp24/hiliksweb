...
export default function Hero3D() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Studio environment map → genuine glossy reflections on the metal
      ...
      scene.add(new THREE.AmbientLight(0x5a6b80, 0.75));
      const d = new THREE.DirectionalLight(0xffffff, 1.35);
      d.position.set(4, 8, 6);
      scene.add(d);
      const pO = new THREE.PointLight(0xF2680E, 5.2, 85); // brand-orange key light
      pO.position.set(7, 3, 10);
      scene.add(pO);
      const pB = new THREE.PointLight(0x0a78a8, 1.4, 55); // darker blue fill
      pB.position.set(-9, -4, 4);
      scene.add(pB);
      // sharp frontal sparkle — tight specular gloss on the metal
      const pF = new THREE.PointLight(0xfff4e0, 2.1, 80);
      ...
      // Bright, glossy, highly metallic spheres in the brand/logo orange (#F2680E)
      ...
      // Darker metallic blue connecting sticks
      ...
      glowSprite('rgba(242,104,14,.9)', 2, 1, 22);
      glowSprite('rgba(0,90,140,.4)', -6, -3, 15);
